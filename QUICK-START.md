# Quick Start Guide - AL Development Collection

> **⚠️ AI-Generated Content Disclaimer**: This toolkit uses GitHub Copilot and generative AI to assist in AL development. Agent responses and code generation results may vary based on context, model versions, and user inputs. Always review and test generated code thoroughly before deploying to production environments.

> **⚠️ Aviso sobre Contenido Generado por IA**: Este toolkit utiliza GitHub Copilot e inteligencia artificial generativa para asistir en el desarrollo AL. Las respuestas de los agentes y los resultados de generación de código pueden variar según el contexto, versiones del modelo y las entradas del usuario. Siempre revise y pruebe el código generado exhaustivamente antes de desplegarlo en entornos de producción.

---

## 🌐 Choose Your Language / Elige Tu Idioma

### 📘 English Version
**Complete Quick Start Guide in English**

👉 **[Read the English Quick Start Guide →](./QUICK-START-en.md)**

---

### 📗 Versión en Español (Castellano)
**Guía de Inicio Rápido Completa en Español**

👉 **[Lee la Guía de Inicio Rápido en Español →](./QUICK-START-es.md)**

---

## 📚 What's Included / Qué Incluye

**Both versions cover / Ambas versiones cubren:**

- ✅ 5-minute installation guide / Guía de instalación de 5 minutos
- ✅ Two main tools: al-architect & al-conductor / Dos herramientas principales
- ✅ Basic workflow for simple, moderate, and complex features / Flujo de trabajo básico
- ✅ Complete example: Loyalty Points System / Ejemplo completo: Sistema de Puntos de Lealtad
- ✅ Useful commands and modes / Comandos útiles y modos
- ✅ Auto-guidelines explanation / Explicación de auto-guidelines
- ✅ Tips for maximum performance / Tips para máximo rendimiento
- ✅ Quick troubleshooting / Troubleshooting rápido

---

**AI-powered AL development in 2 simple steps** for Microsoft Dynamics 365 Business Central.

**Desarrollo AL con IA en 2 pasos simples** para Microsoft Dynamics 365 Business Central.

---

## 📚 Additional Resources / Recursos Adicionales

**For more information, see / Para más información, consulta:**

- **[Complete Framework / Framework Completo](./al-development.md)** - Detailed technical guide / Guía técnica detallada
- **[Reproducible Example / Ejemplo Reproducible](./REPRODUCIBLE-EXAMPLE.md)** - Complete walkthrough / Tutorial completo
- **[Instructions / Instrucciones](./instructions/)** - Auto-guidelines
- **[Workflows](./prompts/README.md)** - Available commands / Comandos disponibles
- **[Modes / Modos](./agents/)** - Specialists / Especialistas

---

**Version / Versión**: 2.7.0  
**Framework**: [AI Native-Instructions Architecture](https://danielmeppiel.github.io/awesome-ai-native/)  
**Last Updated / Última Actualización**: 2025-11-14
